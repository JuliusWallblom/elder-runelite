...
final class Class49 {
	...
	final void method325(Buffer stream) {
		((Class49) this).anInt698 = stream.readUnsignedByte();
	...
	final void method327(Buffer stream) {
		anInt701 = stream.readUnsignedByte();
		anIntArray697 = new int[anInt701];
		anIntArray702 = new int[anInt701];
		for (int i = 0; i < anInt701; i++) {
			anIntArray697[i] = stream.readUnsignedShort();
			anIntArray702[i] = stream.readUnsignedShort();
		}
	}
}
