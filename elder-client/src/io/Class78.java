...
abstract class Class78 {
	...
	static MultilingualString aClass67_1015 = new MultilingualString(" ", ": ", " ", " ");
	...
	static CacheIndex aClass_fs1019;

	static final void method489(byte i, int i_0_, int revision) {
		try {
			if (i > -9)
				method495(null, null, false);
			anInt1011++;
			if (Widget.load_widget(i_0_, revision, 8))
				Class246_Sub15.method1999(false, -1, Widget.get_widget_cache(revision)[i_0_]);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	static final void method495(Object[] objects, int[] is, boolean bool) {
		try {
			...
			throw Class193.method1272(runtimeexception, ("hh.A(" + (objects != null ? "{...}" : "null") + ','
					+ (is != null ? "{...}" : "null") + ',' + bool + ')'));
		}
	}
}
