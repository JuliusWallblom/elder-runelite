...
abstract class Class246_Sub19 extends Node {
	...
	static final void method2067(Color color, int i, Color color_5_, Color color_6_, int i_7_, boolean bool,
			String string) {
		do {
			try {
				anInt4709++;
				try {
					...
					if (bool) {
						...
						graphics.fillRect(0, 0, Client.client_width, Client.client_height);
					}
					if (color_6_ == null)
					...
					try {
						...
						if (i < 98)
							Widget.reset_widget(-110);
						graphics_8_.setColor(color);
						graphics_8_.drawString(string, (304 - 6 * string.length()) / 2, 22);
						graphics.drawImage(Class246_Sub28_Sub22.anImage6633, -152 + (Client.client_width) / 2,
								-18 + Client.client_height / 2, null);
					} catch (Exception exception) {
						int i_9_ = Client.client_width / 2 - 152;
						int i_10_ = Client.client_height / 2 - 18;
					...
					graphics.setFont(Class214.aFont2992);
					graphics.setColor(color);
					graphics.drawString(Class246_Sub28_Sub9.aString6139,
				...
				throw Class193.method1272(runtimeexception,
						("kd.K(" + (color != null ? "{...}" : "null") + ',' + i + ','
								+ (color_5_ != null ? "{...}" : "null") + ',' + (color_6_ != null ? "{...}" : "null")
								+ ',' + i_7_ + ',' + bool + ',' + (string != null ? "{...}" : "null") + ')'));
			}
		} while (false);
	}
	...
	static final void method2070(int i) {
		try {
			...
			if (i != 17569)
				method2070(-36);
			i_11_ += Settings.draw_distance * 128;
			if (Client.sprite_loader.method1727())
				i_11_ += 128;
			Client.sprite_loader.ia(50, i_11_);
		} catch (RuntimeException runtimeexception) {
