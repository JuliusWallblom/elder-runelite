...
final class ka extends Font implements Class_d {
	long nativeid;
	...
	ka(qa var_qa, na var_na, FontMetric class73, LDSprite[] class10s, AbstractSprite[] class_ls) {
		...
		for (int i = 0; i < class10s.length; i++) {
			is[i] = ((LDSprite) class10s[i]).aByteArray157;
			is_4_[i] = ((LDSprite) class10s[i]).anInt156;
			is_5_[i] = ((LDSprite) class10s[i]).anInt155;
			is_6_[i] = ((LDSprite) class10s[i]).anInt158;
			is_7_[i] = ((LDSprite) class10s[i]).anInt162;
		}
		ta(var_qa, var_na, is, ((LDSprite) class10s[0]).anIntArray160, is_4_, is_5_, is_6_, is_7_);
	}
