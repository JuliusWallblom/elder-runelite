...
public final class NativeLibraryManager {
	...
	public static final int writeNativeLibrary(String name) {
		...
		if (!Cache.client_cache.get(IndexConstants.NATIVE_LIBRARIES_INDEX).test("", fullPath)) {
			return -1;
		}

		if (!Cache.client_cache.get(IndexConstants.NATIVE_LIBRARIES_INDEX).method90(-1, fullPath)) {
			int loaded = Cache.client_cache.get(IndexConstants.NATIVE_LIBRARIES_INDEX).method100(fullPath, (byte) 33);
			return loaded;
		}

		byte[] cachedLibraryData = Cache.client_cache.get(IndexConstants.NATIVE_LIBRARIES_INDEX).method96(fullPath, false, "");
	...
	public static void clean() {
		...
		Cache.reset_index(Revision.PRE_EOC_634, "natives");
