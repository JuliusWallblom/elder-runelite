...
final class Class101 {
	...
	static final void decodePlayerUpdate(Stream_Sub1 stream_sub1, int i, int i_0_) {
		try {
			...
			if ((i_0_ ^ 0xffffffff) != (((Buffer) stream_sub1).position ^ 0xffffffff))
				throw new RuntimeException("gpi1 pos:" + ((Buffer) stream_sub1).position + " psize:" + i_0_);
		} catch (RuntimeException runtimeexception) {
	...
	static {
		new MultilingualString("Attempting to join channel...", "Chatraum wird betreten...", "Tentative de connexion au canal...", "Tentando acessar canal...");
		anInt1313 = 0;
