...
final class Class24 {
	...
	static int anInt338;
	static OutgoingPacket CLOSE_INTERFACE_OUT = new OutgoingPacket(69, 0);
	private int anInt340;
	static float aFloat341 = 0.0F;
	static MultilingualString aClass67_342;
			...
			try {
				...
				if (Client.sprite_loader != null) {
					...
					for (int i_2_ = 0; (i_2_ ^ 0xffffffff) > -2049; i_2_++) {
						Player class_r_sub1 = Class143.players[i_2_];
						if (class_r_sub1 != null) {
							((Entity) class_r_sub1).f_pc = null;
							for (int i_3_ = 0; ((((Entity) class_r_sub1).f_qc.length ^ 0xffffffff) < (i_3_ ^ 0xffffffff)); i_3_++)
								((Entity) class_r_sub1).f_qc[i_3_] = null;
						}
					}
					for (int i_4_ = 0; VarcLoader.aClass_r_Sub2Array1002.length > i_4_; i_4_++) {
						Npc class_r_sub2 = VarcLoader.aClass_r_Sub2Array1002[i_4_];
						if (class_r_sub2 != null) {
							for (int i_5_ = 0; ((i_5_ ^ 0xffffffff) > (((Entity) class_r_sub2).f_qc.length ^ 0xffffffff)); i_5_++)
								((Entity) class_r_sub2).f_qc[i_5_] = null;
						}
					}
	...
	static {
		aClass67_342 = new MultilingualString("Loading defaults - ", "Lade Standardeinstellungen - ", "Chargement des param\u00e8tres par d\u00e9faut - ", "Carregando padr\u00f5es - ");
		new MultilingualString("Your clan channel has now been disabled!", "Dein Chatraum ist jetzt ausgeschaltet.", "Votre canal de clan est d\u00e9sactiv\u00e9.", "Seu canal de cl\u00e3 foi desativado!");
		anInt349 = 0;
	}
}
