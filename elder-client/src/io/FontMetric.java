...
final class FontMetric {
	...
	static MultilingualString aClass67_959 = new MultilingualString("Cancel", "Abbrechen", "Annuler", "Cancelar");
	static int anInt960;
	static int anInt961;
	static Class5[] aClass5Array962;

	final int method458(AbstractSprite[] class_ls, int[] is, byte i, String[] strings, String string) {
		try {
			anInt960++;
			if (string == null)
				return 0;
				...
				if ((i_10_ ^ 0xffffffff) == -61)
					...
					if ((i_11_ ^ 0xffffffff) < -1) {
						...
						if (is != null) {
							...
							if ((is[((i_7_ ^ 0xffffffff) > (is.length ^ 0xffffffff) ? i_7_ : is.length - 1)]
			...
			throw Class193.method1272(runtimeexception,
					("gt.G(" + (class_ls != null ? "{...}" : "null") + ',' + (is != null ? "{...}" : "null") + ',' + i
							+ ',' + (strings != null ? "{...}" : "null") + ',' + (string != null ? "{...}" : "null")
							+ ')'));
		}
	}
	...
	final int method461(String string, int i, AbstractSprite[] class_ls) {
		try {
			anInt948++;
			if (string == null)
				return 0;
			...
			throw Class193.method1272(runtimeexception, ("gt.F(" + (string != null ? "{...}" : "null") + ',' + i + ','
					+ (class_ls != null ? "{...}" : "null") + ')'));
		}
	}
	...
	static final Class59_Sub4 method463(int i, int i_24_, int i_25_) {
		Tile class147 = Class106.tiles[i][i_24_][i_25_];
		if (class147 == null)
			return null;
		return ((Tile) class147).aClass59_Sub4_2152;
	}

	final int method464(String string, int i, int i_26_, AbstractSprite[] class_ls, int i_27_) {
		try {
			anInt945++;
			if (i_26_ == (i_27_ ^ 0xffffffff))
				i_27_ = ((FontMetric) this).anInt955;
			int i_28_ = method458(class_ls, new int[] { i }, (byte) -52, Class171.aStringArray2426, string);
			int i_29_ = i_27_ * (-1 + i_28_);
			return (((FontMetric) this).anInt951 - (-i_29_ - ((FontMetric) this).anInt946));
		} catch (RuntimeException runtimeexception) {
			throw Class193.method1272(runtimeexception, ("gt.C(" + (string != null ? "{...}" : "null") + ',' + i + ','
					+ i_26_ + ',' + (class_ls != null ? "{...}" : "null") + ',' + i_27_ + ')'));
		}
	}

	final String method465(int i, AbstractSprite[] class_ls, String string, int i_30_) {
		try {
			anInt952++;
			...
			throw Class193.method1272(runtimeexception, ("gt.J(" + i + ',' + (class_ls != null ? "{...}" : "null") + ','
					+ (string != null ? "{...}" : "null") + ',' + i_30_ + ')'));
		}
	}

	final int method466(int i, AbstractSprite[] class_ls, int i_40_, String string) {
		try {
			anInt949++;
			if (i < 59)
				aClass67_959 = null;
			...
			throw Class193.method1272(runtimeexception, ("gt.B(" + i + ',' + (class_ls != null ? "{...}" : "null") + ','
	...
	final int method468(byte i, String string, int i_42_, AbstractSprite[] class_ls) {
		try {
			anInt950++;
			...
			throw Class193.method1272(runtimeexception, ("gt.A(" + i + ',' + (string != null ? "{...}" : "null") + ','
					+ i_42_ + ',' + (class_ls != null ? "{...}" : "null") + ')'));
		}
	}
		...
		try {
			...
			if (bool) {
				...
				for (int i_49_ = 0; (i_49_ ^ 0xffffffff) > -257; i_49_++)
					is_48_[i_49_] = stream.readUnsignedByte();
				int[] is_50_ = new int[256];
				for (int i_51_ = 0; i_51_ < 256; i_51_++)
					is_50_[i_51_] = stream.readUnsignedByte();
					...
					if (i_60_ != 32 && i_60_ != 160) {
						for (int i_61_ = 0; (i_61_ ^ 0xffffffff) > -257; i_61_++) {
							if (i_61_ != 32 && (i_61_ ^ 0xffffffff) != -161)
								aByteArrayArray953[i_60_][i_61_] = (byte) (Class59_Sub3.method2171(i_61_, (byte) -79,
										is_50_, is_56_, aByteArray958, i_60_, is_48_, is_52_));
						}
					}
				}
				((FontMetric) this).anInt955 = is_48_[32] + is_50_[32];
			} else
				((FontMetric) this).anInt955 = stream.readUnsignedByte();
			stream.readUnsignedByte();
			stream.readUnsignedByte();
			((FontMetric) this).anInt951 = stream.readUnsignedByte();
			((FontMetric) this).anInt946 = stream.readUnsignedByte();
		} catch (RuntimeException runtimeexception) {
