...
final class Class246_Sub28_Sub23 extends TextureOperation {
	...
	static IncomingPacket SEND_GRAPHICS = new IncomingPacket(61, 5);
	private int anInt6655;
	static AbstractSprite aClass_l6656;
									...
									while_90_: do {
										do {
											if (i_1_ != 0) {
												...
												anInt6648 = stream.readUnsignedByte();
												break while_95_;
											}
											anInt6655 = stream.readUnsignedByte();
											break while_95_;
										} while (false);
										anInt6636 = stream.readUnsignedShort();
										break while_95_;
									} while (false);
									anInt6647 = stream.readUnsignedShort();
									break while_95_;
								} while (false);
								anInt6644 = stream.readUnsignedShort();
								break while_95_;
							} while (false);
							anInt6652 = stream.readUnsignedShort();
							break while_95_;
						} while (false);
						anInt6637 = stream.readUnsignedShort();
						break while_95_;
					} while (false);
					anInt6649 = stream.readUnsignedShort();
				} while (false);
				if (i == 4)
					break;
				method2134(false);
	...
	final int[] method2135(int i, int i_2_) {
		try {
			...
			if (((Class66) ((TextureOperation) this).aClass66_4938).aBool887) {
				int i_4_ = 0;
				int i_5_;
				for (i_5_ = (Varc.anIntArray6245[i_2_] - -anInt6652); (i_5_ ^ 0xffffffff) > -1; i_5_ += 4096) {
					/* empty */
				}
	...
	static final byte[] method2727(float f, float f_16_, float f_17_, float f_18_, int i, Class64 class64, int i_19_, byte i_20_, float f_21_, int i_22_, int i_23_) {
		try {
			...
			MapFunction.method946(is, f_17_, f, 0, i_23_, i_22_, f_18_, f_16_, i, 127, i_19_, f_21_, class64);
			return is;
		} catch (RuntimeException runtimeexception) {
	...
	static {
		new MultilingualString("Your ignore list is full. Max of 100 users.", "Deine Ignorieren-Liste ist voll, du kannst nur 100 Spieler darauf eintragen.", "Votre liste noire est pleine (100 noms maximum).", "Sua lista de ignorados est\u00e1 cheia. O limite \u00e9 100 usu\u00e1rios.");
	}
}
