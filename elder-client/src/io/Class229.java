...
final class Class229 {
	...
	static final Class229 method1467(CacheIndex class_fs, int i, int i_8_) {
		byte[] is = class_fs.get_file(i_8_, i);
		if (is == null)
			return null;
		return new Class229(new Buffer(is));
	}
	...
	private Class229(Buffer stream) {
		...
		anInt3140 = stream.readUnsignedShort();
		anInt3141 = stream.readUnsignedShort();
	}
