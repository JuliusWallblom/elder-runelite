...
final class Class246_Sub28_Sub2 extends TextureOperation {
			...
			try {
				...
				while_116_: do {
					do {
						if ((i_8_ ^ 0xffffffff) != -3) {
							...
							for (i_8_ = 0; (i_8_ ^ 0xffffffff) > -258; i_8_++) {
								int i_9_ = i_8_ << 4;
								int i_10_;
								for (i_10_ = 1; ((anIntArrayArray5800.length - 1 ^ 0xffffffff) < (i_10_
										^ 0xffffffff)); i_10_++) {
						...
						for (i_8_ = 0; (i_8_ ^ 0xffffffff) > -258; i_8_++) {
							...
							int i_30_ = (-(StructType.anIntArray6855[(i_29_ & 0x1ff5) >> 5]) + 4096 >> 1);
					...
					for (i_8_ = 0; i_8_ < 257; i_8_++) {
						int i_33_ = i_8_ << 4;
						int i_34_;
						for (i_34_ = 1; ((i_34_ ^ 0xffffffff) > (-1 + anIntArrayArray5800.length
								^ 0xffffffff)); i_34_++) {
	...
	final void method2143(int i, int i_44_, Buffer stream) {
		do {
			try {
				anInt5804++;
				if ((i_44_ ^ 0xffffffff) == -1) {
					anInt5797 = stream.readUnsignedByte();
					anIntArrayArray5800 = new int[stream.readUnsignedByte()][2];
					for (int i_45_ = 0; i_45_ < anIntArrayArray5800.length; i_45_++) {
						anIntArrayArray5800[i_45_][0] = stream.readUnsignedShort();
						anIntArrayArray5800[i_45_][1] = stream.readUnsignedShort();
					}
				}
				if (i == 4)
					break;
	...
	static {
		new MultilingualString("clan_chat", "clanchat", "conversation_clan", "clan_chat");
