...
final class Varbit {
			...
			try {
				...
				for (;;) {
					...
					if ((opcode ^ 0xffffffff) == -1)
						break;
					process_op(opcode, stream, 15902);
				}
				if (i == -25)
					break;
	...
	private final void process_op(int opcode, Buffer stream, int i_3_) {
		do {
			try {
				if (opcode == 1) {
					((Varbit) this).index = stream.readUnsignedShort();
					((Varbit) this).least_significant_bit = stream.readUnsignedByte();
					((Varbit) this).most_significant_bit = stream.readUnsignedByte();
				}
				anInt1528++;
				if (i_3_ == 15902)
					break;
				...
				throw Class193.method1272(runtimeexception, ("ki.D(" + opcode + ',' + (stream != null ? "{...}" : "null") + ',' + i_3_ + ')'));
			}
			break;
		} while (false);
	}
	...
	static {
		new MultilingualString("That player is offline, or has privacy mode enabled.", "Dieser Spieler ist offline oder hat den Privatsph\u00e4ren-Modus aktiviert.", "Ce joueur est d\u00e9connect\u00e9 ou en mode priv\u00e9.", "O jogador est\u00e1 offline ou est\u00e1 com o modo de privacidade ativado.");
