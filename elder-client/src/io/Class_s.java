...
final class Class_s extends Font implements Class_d {
	long nativeid;

	final native void OA(char c, int i, int i_0_, int i_1_, boolean bool);

	Class_s(qa var_qa, na var_na, FontMetric class73, LDSprite[] class10s, AbstractSprite[] class_ls) {
		...
		for (int i = 0; i < class10s.length; i++) {
			...
			is_2_[i] = ((LDSprite) class10s[i]).anInt156;
			is_3_[i] = ((LDSprite) class10s[i]).anInt155;
			is_4_[i] = ((LDSprite) class10s[i]).anInt158;
			is_5_[i] = ((LDSprite) class10s[i]).anInt162;
		}
