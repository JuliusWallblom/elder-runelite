...
final class Class246_Sub28_Sub4 extends TextureOperation {
	...
	final void method2143(int i, int i_11_, Buffer stream) {
		try {
			if (i_11_ == 0) {
				int i_12_ = stream.readUnsignedByte();
				if ((i_12_ ^ 0xffffffff) == -1) {
					anIntArrayArray5972 = new int[stream.readUnsignedByte()][4];
					for (int i_13_ = 0; ((i_13_ ^ 0xffffffff) > (anIntArrayArray5972.length ^ 0xffffffff)); i_13_++) {
						anIntArrayArray5972[i_13_][0] = stream.readUnsignedShort();
						anIntArrayArray5972[i_13_][1] = stream.readUnsignedByte() << 4;
						anIntArrayArray5972[i_13_][2] = stream.readUnsignedByte() << 4;
						anIntArrayArray5972[i_13_][3] = stream.readUnsignedByte() << 4;
					}
				} else
	...
	static final void method2537() {
		...
		if (Class1.aClass147ArrayArrayArray4 != null) {
			for (int i = 0; i < Class1.aClass147ArrayArrayArray4.length; i++) {
				for (int i_16_ = 0; i_16_ < Class240.xSize; i_16_++) {
					for (int i_17_ = 0; i_17_ < HDModel.ySize; i_17_++) {
		...
		if (StructLoader.aClass174Array811 != null) {
			for (int i = 0; i < Class_c.anInt1195; i++)
				StructLoader.aClass174Array811[i] = null;
			for (int i = 0; i < Class154_Sub1.anInt4803; i++) {
				for (int i_18_ = 0; i_18_ < Class240.xSize; i_18_++) {
					for (int i_19_ = 0; i_19_ < HDModel.ySize; i_19_++)
						MapRegion.aLongArrayArrayArray3648[i][i_18_][i_19_] = 0L;
				}
			}
			Class_c.anInt1195 = 0;
		}
	...
	final int[][] method2139(byte i, int i_22_) {
		try {
			anInt5967++;
			int[][] is = ((TextureOperation) this).aClass136_4933.method922((byte) -118, i_22_);
			if (i <= 33)
