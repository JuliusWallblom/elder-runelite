...
final class Class239 {
	...
	static final String method1536(int i, int i_0_) {
		try {
			...
			if (string.length() > 9)
				return (" <col=00ff80>" + string.substring(0, string.length() - 8) + Class246_Sub43.aClass67_5598.get(Preferences.language_code) + " (" + string + ")</col>");
			if ((string.length() ^ 0xffffffff) < -7)
				return (" <col=ffffff>" + string.substring(0, string.length() - 4) + Class115.aClass67_1606.get(Preferences.language_code) + " (" + string + ")</col>");
	...
	static {
		aClass13_3375 = new IncomingPacket(75, 18);
	}
}
