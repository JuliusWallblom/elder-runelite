...
final class Class84 {
	...
	private IterableNodeHashTable aClass85_1099;
	...
	final void method537(byte i, Buffer stream) {
		do {
			try {
				for (;;) {
				...
				throw Class193.method1272(runtimeexception,
						("hv.F(" + i + ',' + (stream != null ? "{...}" : "null") + ')'));
			}
			break;
		} while (false);
	}

	static final TextureOperation method538(Buffer stream, byte i) {
		try {
			...
			TextureOperation class246_sub28 = Class40_Sub1.method1770(i_4_, 35);
			((TextureOperation) class246_sub28).anInt4932 = stream.readUnsignedByte();
	...
	private final void method539(Buffer stream, int i, int i_8_) {
		try {
			anInt1091++;
			if (i_8_ <= 45)
			...
			if ((i ^ 0xffffffff) == -2)
				...
				if (i != 4) {
								...
								if (i == 10) {
									...
									if (i == 14) {
										...
										for (int i_12_ = 0; ((i_12_ ^ 0xffffffff) > (i_11_ ^ 0xffffffff)); i_12_++) {
											anIntArrayArray1111[i_12_][0] = stream.readUnsignedByte();
											anIntArrayArray1111[i_12_][1] = stream.readUnsignedByte();
										}
									} else if (i == 15)
										stream.readUnsignedShort();
									else if ((i ^ 0xffffffff) == -18)
										((Class84) this).anInt1107 = stream.readUnsignedShort();
										...
										if ((i ^ 0xffffffff) != -20) {
											if (i == 249) {
												...
												if (aClass85_1099 == null) {
													...
													aClass85_1099 = new IterableNodeHashTable(i_14_);
												}
												for (int i_15_ = 0; ((i_15_ ^ 0xffffffff) > (i_13_
														^ 0xffffffff)); i_15_++) {
													boolean bool = (stream.readUnsignedByte() ^ 0xffffffff) == -2;
											...
											for (int i_18_ = 0; ((i_17_ ^ 0xffffffff) < (i_18_
													^ 0xffffffff)); i_18_++) {
									...
									for (int i_22_ = 0; ((i_21_ ^ 0xffffffff) < (i_22_ ^ 0xffffffff)); i_22_++)
										anIntArray1108[i_22_] = stream.readUnsignedShort();
								}
							} else
								stream.readUnsignedByte();
						}
					...
					for (int i_24_ = 0; i_23_ > i_24_; i_24_++) {
						anIntArrayArray1109[i_24_][0] = stream.readUnsignedShort();
				...
				for (int i_26_ = 0; i_26_ < i_25_; i_26_++) {
					anIntArrayArray1092[i_26_][0] = stream.readUnsignedShort();
	...
	static {
		new MultilingualString("Invalid name", "Unzul\u00e4ssiger Name!", "Nom incorrect", "Nome inv\u00e1lido");
